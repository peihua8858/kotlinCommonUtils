package com.peihua8858.android.tools.timer
...
class TaskTimer(var period: Long) : LifecycleObserver {
            ...
            override fun dispatchMessage(msg: Message) {
                ...
                if (callback != null) {
                    ...
                    if (callback is TimerTask) {
                        if (callback.state == TimerTask.Companion.CANCELLED) {
                            return
                        } else if (callback.period == 0L) {
                            callback.state = TimerTask.Companion.EXECUTED
                            return
                        }
    ...
    fun schedule(task: Runnable, delay: Long, period: Long) {
        ...
        if (task is TimerTask) {
            task.state = TimerTask.Companion.SCHEDULED
            task.period = period
        }
        if (!isStarted()) {
            onStart()
